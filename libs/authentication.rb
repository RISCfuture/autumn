module Autumn
  ...
  # {Leaf} class), the options given are used to choose the correct class within
  # this module to serve as the authenticator for that leaf.
  #
  ...
  # -------------------------------
  #
  # To define your own authenticator, subclass Base as a new class in the
  # Authentication module. Implement the methods defined in the Base class docs,
  # then adjust your configuration to use your new authenticator.
  ...
  module Authentication
    ...
    # override the {#authenticate} method. You should also override the
    # {#initialize} method if you need to store any options or other data for
    # later use.
    #
    # The authentication module will become a stem listener, so see
    # {Stem#add_listener} for information on other methods you can implement.
    ...
    class Base
      ...
      #
      # @param [Hash<Symbol, Object>] options The authenticator configuration.
      ...
      #
      # @param [Stem] stem The Stem on which the Leaf is running.
      # @param [String] channel The channel name.
      # @param [Hash] sender A sender hash (see {Stem} class documentation).
      # @param [Leaf] leaf The Leaf that is requesting user authentication.
    ...
    # Authenticates by a password provided in secret. When a user `PRIVMSG`'s
    # the leaf the correct password, the leaf adds that user's nick to a list of
    ...
    class Password < Base
      ...
      # @private
      def irc_kick_event(stem, _, arguments)
        @an_lock.synchronize { revoke stem, arguments[:nick] }
      end

      # @private
      def irc_quit_event(stem, sender, _)
